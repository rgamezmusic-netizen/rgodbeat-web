...
let ytPlayer;
let backgroundMusicVolume = 30;
let isPlayerReady = false;
let hasUnmuted = false; // Track if we've already unmuted
...
window.onYouTubeIframeAPIReady = function () {
    ytPlayer = new YT.Player('ytPlayer', {
        height: '0',
        width: '0',
        videoId: 'vaPS8r39ois', // First video from playlist
        playerVars: {
            'autoplay': 1,
            ...
            'loop': 1,
            'listType': 'playlist',
            'list': 'PLmNezEeokSZ4yltojerC0rAkmzt0uo282', // ✅ Correct playlist ID
            'playsinline': 1,
            'enablejsapi': 1,
...
function onPlayerReady(event) {
    isPlayerReady = true;
    // Set playback quality to 'small' to save bandwidth
    ytPlayer.setPlaybackQuality('small');

    // Start playing muted (browsers allow this)
    ytPlayer.playVideo();
    console.log('YouTube player started (muted, low quality)');

    // Update button to show muted state
    ...
    document.addEventListener('click', function unmuteOnInteraction() {
        if (!hasUnmuted && isPlayerReady) {
            ytPlayer.unMute();
            ytPlayer.setVolume(backgroundMusicVolume);
            hasUnmuted = true;

            // Update button to show unmuted state
            if (playPauseBtn) {
                playPauseBtn.textContent = '🔊';
            }

            console.log('YouTube player unmuted on user interaction');
...
function onPlayerStateChange(event) {
    // Just log state changes for debugging
    // console.log('Player state:', event.data);
    // Ensure quality stays low on state changes (like next video)
...
function onPlayerError(event) {
    ...
    // If playlist fails, try a fallback video or just hide the player
}
    ...
    setTimeout(() => {
        ...
        if (playPauseBtn) {
            playPauseBtn.addEventListener('click', () => {
                ...
                if (isMuted) {
                    ytPlayer.unMute();
                    smoothVolumeChange(ytPlayer, backgroundMusicVolume, 500);
                    playPauseBtn.textContent = '🔊';
                    hasUnmuted = true;
                } else {
                    ytPlayer.mute();
        ...
        if (volumeSlider) {
            volumeSlider.addEventListener('input', (e) => {
                backgroundMusicVolume = parseInt(e.target.value);
                if (volumeValue) volumeValue.textContent = backgroundMusicVolume + '%';
                if (isPlayerReady) {
                    ytPlayer.setVolume(backgroundMusicVolume);
                }
            });
        ...
        if (playerToggle && playerBody) {
            playerToggle.addEventListener('click', () => {
                playerBody.classList.toggle('collapsed');
                playerToggle.textContent = playerBody.classList.contains('collapsed') ? '+' : '─';
            });
        }
    }, 1000);
});

// --- AUTO-DUCKING: Lower background music when beat plays ---
const originalBeatGridClick = document.querySelector('.beat-grid');
if (originalBeatGridClick) {
    originalBeatGridClick.addEventListener('click', (e) => {
        if (e.target.classList.contains('play-btn-large') && isPlayerReady) {
            const audioId = e.target.getAttribute('data-audio');
            const audio = document.getElementById(audioId);

            if (audio) {
                // When beat starts playing, mute background music completely (but keep playing)
                audio.addEventListener('play', () => {
                    // Mute completely for better listening experience
                    smoothVolumeChange(ytPlayer, 0, 500);
                });

                // When beat is paused, restore background music volume
                audio.addEventListener('pause', () => {
                    smoothVolumeChange(ytPlayer, backgroundMusicVolume, 1000);
                });

                // When beat ends, restore background music volume
                audio.addEventListener('ended', () => {
                    smoothVolumeChange(ytPlayer, backgroundMusicVolume, 1000);
                });
            }
...
function smoothVolumeChange(player, targetVolume, duration) {
