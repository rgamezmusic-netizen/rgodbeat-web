    ...
    <header className="sticky top-0 z-30 flex items-center justify-between px-3 sm:px-6 py-2 bg-[#09090b]/95 backdrop-blur-md border-b border-zinc-800/80">
      ...
      <div className="flex items-center gap-2.5 min-w-0">
        ...
        {accessStatus?.hasActivePass ? (
          <div
            className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-[10px] sm:text-xs font-mono font-bold"
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-full bg-amber-500/15 hover:bg-amber-500/25 border border-amber-500/40 text-amber-300 text-[10px] sm:text-xs font-mono font-bold cursor-pointer transition-all active:scale-95"
        ...
        {currentBeatTitle && (
          <div
            ...
            className="hidden md:flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-zinc-900/90 border border-zinc-750 hover:border-amber-500/50 cursor-pointer text-xs font-mono transition-colors shadow-sm"
      ...
      {/* Zone 2: View Switcher (Estudio vs Editor) & Physical Undo/Redo Dock */}
      <div className="flex items-center gap-2 sm:gap-3">
        ...
        <div className="flex items-center bg-zinc-900/90 p-1 rounded-xl border border-zinc-800 shadow-inner">
          <button
            onClick={onUndo}
            disabled={!canUndo}
            className={`flex items-center gap-1 px-2 sm:px-2.5 py-1 rounded-lg text-xs font-mono font-medium transition-all ${
              canUndo
        ...
        {/* View Switcher (Reproductor vs Espacio de Edición) */}
        <div className="flex items-center bg-zinc-900/90 p-0.5 rounded-xl border border-zinc-800">
          ...
          >
            ...
            <span className="hidden xs:inline sm:inline">Estudio</span>
          </button>
      ...
      <div className="flex items-center gap-1.5 sm:gap-2">
        <button
          onClick={onOpenLoadBeat}
          className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-medium bg-zinc-900 text-zinc-300 border border-zinc-800 hover:bg-zinc-800 hover:border-zinc-700 transition-all"
        ...
        {isRecording && (
          <div className="flex items-center gap-1 px-2 py-1 rounded-full bg-red-950/70 border border-red-500/40 text-red-400 text-xs font-mono animate-pulse">
        ...
        <button
          ...
          className={`flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all shadow-sm ${
            hasRecordings
